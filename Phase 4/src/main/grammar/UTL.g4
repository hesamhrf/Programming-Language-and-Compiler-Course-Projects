...
printStatement returns [PrintStmt printRet] :
    p=PRINT LPAREN exp1=expression { $printRet = new PrintStmt($exp1.expressionRet); $printRet.setLine($p.getLine()); } RPAREN SEMICOLON;
...
espetialFunction returns [Identifier name]: {$name = new Identifier(null);}(n=REFRESH_RATE | n=CONNECT | n=OBSERVE | n=GET_CANDLE | n=TERMINATE){$name.setName($n.text);$name.setLine($n.line);};
...
assign returns [BinaryOperator op]:
    (ASSIGN {$op = BinaryOperator.ASSIGN;} | ADD_ASSIGN {$op = BinaryOperator.ADD_ASSIGN;} | SUB_ASSIGN {$op = BinaryOperator.SUB_ASSIGN;} | MUL_ASSIGN {$op = BinaryOperator.MUL_ASSIGN;} | DIV_ASSIGN {$op = BinaryOperator.DIV_ASSIGN;} | MOD_ASSIGN {$op = BinaryOperator.MOD_ASSIGN;});
